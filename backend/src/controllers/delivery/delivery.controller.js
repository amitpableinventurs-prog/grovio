...
// POST /delivery/jobs/:id/otp/verify { otp } -> validates the hub picker's handover OTP. This is
// the only way an order can move to 'picked_up' (see order.service.js).
...
// POST /delivery/jobs/:id/scan { qrToken, deviceId?, location?: { lat, lng } } -> scans the hub
// picker's handover QR code. Alternative to POST .../otp/verify above — either one completes the
// same handover (order -> picked_up). Every attempt, successful or not, is recorded in ScannerLog.
